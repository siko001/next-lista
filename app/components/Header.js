        ...
        <>        {/* normal header */}
            ...
            {/* Register user Navigation */}
            {
                isRegistered && <div className={" py-4 md:py-6 px-4 md:px-8 xl:px-16 flex justify-between items-center gap-12"}>
                    <Link href={"/"} className={"font-bold text-3xl"}>Lista</Link>
                    <div className="flex gap-y-2 gap-x-2 md:gap-4 items-center flex-wrap justify-end">
                        ...
                        <Link className="text-white  py-3 px-6 xl:px-10 text-xs overflow-hidden md:text-base group font-bold rounded-full bg-blue-800 relative" href={"/logout"} >

                            <p className="relative z-20 group-hover:text-black duration-700">
                                Logout
                            </p>

                            <div className="absolute w-0 h-0 rounded-full group-hover:w-full group-hover:h-full transition-all top-[50%] z-10 left-[50%] -translate-x-1/2 -translate-y-1/2 duration-200 bg-[#21ba9c]">
                            </div>
