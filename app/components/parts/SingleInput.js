...
export default function SingleInput() {
    ...
    return (
        <div className={"flex flex-col items-start gap-2"}>
            <div
                ref={parentRef}
                className="w-full items-start relative flex items-center justify-between bg-gray-200 h-[50px] rounded-md border border-blue-500 overflow-hidden"
            >
                <input
                    ref={inputRef}
                    onChange={handleChange}
                    className="focus:outline-none w-full flex py-3 px-2 items-center"
                    type="text"
                    placeholder="Give your list-a name"
                    maxLength={maxLength}
                />
                <p className="w-min whitespace-nowrap bg-white h-full flex items-center px-2 text-xs font-bold font-quicksand ">
                    <span
                        className={`${
                            hasTyped &&
                            length < 3 &&
            ...
            {errors.message && (
                <p className=" py-2 px-2 bg-red-400 rounded-sm text-white flex gap-1 items-center text-xs">
